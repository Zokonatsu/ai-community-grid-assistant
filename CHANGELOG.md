...
### 修复：地图选点（指针坏图 + 搜索不跳转/不可选）
- **指针**：补齐 Leaflet 默认 marker 图标（`static/vendor/leaflet/images/`），消除“?”坏图，指针恢复为可拖拽红针。
- **搜索**：改为「输入即联想」，并按当前地图中心/定位作为 `location` 偏置，高德优先返回本地且带坐标的结果；按回车自动选中最近一条，仍可点选候选列表。
- 后端 `/api/geo/search` 新增可选 `location`/`city` 参数；`/api/geo/regeo` 与后台展示逻辑不变。

### 修复：平均处理时长不再显示 0 分钟
- **根因**：「平均处理时长」误用了「上班时段有效时长」切片函数，事件在非上班时段（如凌晨 01:17–01:30）被受理并完成时，有效时长被切片为 0 分钟。
- **修正**：处理时长改按**实际自然分钟**计算（首次处理时间 → 完成时间），不再按上班时段/周末切片；每日 SLA 指标与 AI 周报口径同步修正。
- **说明**：上班时间仅用于展示/配置（为后续 AI 周报等复用），**不参与处理时长统计**；仅已办结工单参与，超管自己完成的工单不纳入。
