...
模块接口约定（参见 INTERFACE.md）：
  ...
  输出: {
      "address": "具体地址",
      "event_type": "物业维修/环境卫生/安全隐患/邻里纠纷/公共设施/医疗急救/其他",
      "urgency": "高/中/低",
...
_EMERGENCY_FIRE_RE = re.compile(
    r"火灾|起火|着火|燃气泄漏|煤气泄漏|煤气味|燃气味|煤气|燃气|被困|反锁|门锁|锁住|打不开|出不去|困在|爆炸|救火|坍塌|电梯困人|高空坠物|严重交通事故",
    re.IGNORECASE,
)
...
class ReceiveState(TypedDict):
    ...
    字段说明：
        ...
        event_type:  事件分类，限定为：物业维修/环境卫生/安全隐患/邻里纠纷/公共设施/医疗急救/其他/无效输入/API异常/待审核。
...
- "割腕" → is_valid=true，event_type="医疗急救"，urgency="高"，scene_tag="生命急救"（生命安全紧急情况，网格员必须介入上报）
...
4. event_type（字符串）：事件类型，只能从以下类别中选择一项。选择时必须结合各部门职责范围进行判断：
   ...
   第二：涉及人身安全威胁、治安犯罪、火灾燃气 → 安全隐患
   第三：居民之间冲突、矛盾、干扰 → 邻里纠纷
   第四：房屋/设备故障需维修 → 物业维修
   第五：公共区域清洁/绿化/垃圾 → 环境卫生
   第六：小区公共基础设施损坏 → 公共设施
   第七：确实无法归入以上六类 → 其他

   - 医疗急救（120急救中心）
     核心：人员伤病、中毒、昏迷、突发疾病，或居民明确要求叫救护车的生命急救场景。
     不属于：设施故障（无人员受伤）、普通口角、停车占位。
     边界：心脏病发作（属于）| 电梯故障困人（不属于，归119消防）
     举例：心脏病发作、胸痛、呼吸困难、晕倒、昏迷、食物中毒、煤气中毒、外伤出血、骨折、抽搐、孕妇临产、割腕、自杀。
   ...
   - 安全隐患（安保部）
     核心：火灾风险、燃气泄漏、治安事件、盗窃、高空坠物等需安保介入的危险情况。
     不属于：普通口角（无肢体冲突）、停车占位（无破坏）、设施故障（无危险）、人员伤病（归医疗急救）。
     边界：车辆被刮且怀疑故意破坏/盗窃（属于）| 车辆被刮且邻里矛盾（归邻里纠纷）| 单纯停车占位（归邻里纠纷）
     举例：社区治安、安全隐患排查、防盗防骗、可疑人员、火灾风险、燃气泄漏、盗窃、高空坠物
     重要提示：未明确提及"故意破坏、盗窃、可疑人员"等治安要素，仅提及"车辆被刮/碰撞"，优先归"邻里纠纷"；人员伤病类优先归"医疗急救"。
