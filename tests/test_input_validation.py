...
def test_suite():
    ...
    # ---- 1.2 长度不足 2（仅 1 字拒绝；2 字交 LLM 语义判断）----
    for name, inp in [
        ("single char 'a'", "a"),
        ("single char '好'", "好"),
    ]:
    ...
    # ---- 1.2b 2 字输入放行（合法短报：漏水/煤气/停电 等交 LLM）----
    for name, inp in [
        ("two chars 'ab'", "ab"),
        ("two Chinese '吃饭'", "吃饭"),
        ("two Chinese '漏水'", "漏水"),
        ...
        actual = _is_valid_input(inp)
        if actual == True:
            results.add_pass(f"1.2b {name}", f"'{inp}' (len={len(inp)}) -> pass to LLM")
        else:
            results.add_fail(f"1.2b {name}", "True", str(actual))
    
    # ---- 1.2c 迷信/超自然内容直接拒绝（防 LLM 随机判定）----
    for name, inp in [
        ("有鬼 '我家有鬼'", "我家有鬼"),
        ("闹鬼 '楼下闹鬼'", "楼下闹鬼"),
        ("妖怪 '家里有妖怪'", "家里有妖怪"),
        ...
        actual = _is_valid_input(inp)
        if actual == False:
            results.add_pass(f"1.2c {name}", f"'{inp}' -> rejected")
        else:
            results.add_fail(f"1.2c {name}", "False", str(actual))
    ...
    # 清理临时数据目录（pytest 模式下由 test_suite 自行清理；直跑由 teardown_test_env 兜底）
    shutil.rmtree(TEST_DATA_DIR, ignore_errors=True)
