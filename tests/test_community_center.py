...
测试范围：
  ...
  6. 保存新中心后，住户注册定位判定跟随新中心（范围内 verified；越界注册被拒绝）
"""
...
def test_suite():
    ...
    check("4.2 新中心外注册被拒绝（T20260819-002）", (not reg_out.get("success"))
          and reg_out.get("error") == "当前定位不在小区范围内，无法注册", f"err={reg_out.get('error')}")
    ...
    out_names = [u["username"] for u in users_admin]
    check("4.4 越界注册未创建住户（列表不含 res_out_new）", "res_out_new" not in out_names, f"names={out_names}")
