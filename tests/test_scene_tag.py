...
def test_suite():
    ...
    if not token:
        ...
        raise AssertionError("无法注册/登录测试用户")
    ...
    case_dispatch_directly("生命急救", "安全隐患", "高", "120医疗急救中心（外部资源）",
                           "A1 生命急救场景 → 派单给急救中心")
    ...
    case_dispatch_directly("常规", "安全隐患", "高", "[紧急]安保部",
                           "A4 常规/安全隐患/高紧急 → 派单给[紧急]安保部")
    # A5: 常规 + 低紧急 + 其他 → 综合部
    case_dispatch_directly("常规", "其他", "低", "综合部",
                           "A5 常规/其他/低紧急 → 派单给综合部")
    # A6: 常规 + 环境卫生 + 中紧急 → 环卫部
    case_dispatch_directly("常规", "环境卫生", "中", "环卫部",
                           "A6 常规/环境卫生/中紧急 → 派单给环卫部")
    # A7: 常规 + 邻里纠纷 + 中紧急 → 调解员
    case_dispatch_directly("常规", "邻里纠纷", "中", "调解员",
                           "A7 常规/邻里纠纷/中紧急 → 派单给调解员")
    # A8: 常规 + 公共设施 + 低紧急 → 工程部
    case_dispatch_directly("常规", "公共设施", "低", "工程部",
                           "A8 常规/公共设施/低紧急 → 派单给工程部")
    # A9: 生命急救场景不受 urgency 影响（已经是外部资源，不加[紧急]前缀）
    case_dispatch_directly("生命急救", "其他", "低", "120医疗急救中心（外部资源）",
    ...
    def case_record_persistence(scene_tag, handler, description, user_id, test_name):
    ...
    case_record_persistence("生命急救", "120医疗急救中心（外部资源）", "有人晕倒了", user_id, "B1 生命急救持久化")
    case_record_persistence("紧急救援", "119消防急救中心（外部资源）", "电梯困人了", user_id, "B2 紧急救援持久化")
    case_record_persistence("常规", "物业部", "下水道堵了", user_id, "B3 常规场景持久化")
    ...
    def case_full_pipeline(description, expected_scene_tag, expected_handler, test_name):
        """
    ...
    case_full_pipeline("有人晕倒了", "生命急救", "120医疗急救中心（外部资源）",
                       "C1 人员晕倒 → 生命急救 → 急救中心")
    
    # C2: 受伤大出血 → 生命急救 → 120医疗急救中心（外部资源）
    case_full_pipeline("3号楼有人受伤大出血", "生命急救", "120医疗急救中心（外部资源）",
                       "C2 受伤大出血 → 生命急救 → 急救中心")
    
    # C3: 电梯困人 → 紧急救援 → 119消防急救中心（外部资源）
    case_full_pipeline("电梯困人了", "紧急救援", "119消防急救中心（外部资源）",
                       "C3 电梯困人 → 紧急救援 → 应急救援队")
    
    # C4: 火灾 → 紧急救援 → 119消防急救中心（外部资源）
    case_full_pipeline("小区门口发生火灾", "紧急救援", "119消防急救中心（外部资源）",
                       "C4 火灾 → 紧急救援 → 应急救援队")
    
    # C5: 燃气泄漏 → 紧急救援 → 119消防急救中心（外部资源）
    case_full_pipeline("燃气泄漏了", "紧急救援", "119消防急救中心（外部资源）",
                       "C5 燃气泄漏 → 紧急救援 → 应急救援队")
    
    # C6: 下水道堵塞 → 常规 → 物业部
    case_full_pipeline("我家楼下下水道堵了", "常规", "物业部",
                       "C6 下水道堵塞 → 常规 → 物业部")
    
    # C7: 路灯损坏 → 常规 → 工程部
    case_full_pipeline("小区路灯坏了", "常规", "工程部",
                       "C7 路灯损坏 → 常规 → 工程部")
    
    # C8: 噪音扰民 → 常规 → 调解员
    case_full_pipeline("楼上邻居半夜噪音扰民", "常规", "调解员",
                       "C8 噪音扰民 → 常规 → 调解员")
    ...
    case_unknown_scene_tag()
    
    
    # E2: receive_agent 防御性校验 - 非预期 scene_tag 回退为常规
    def case_receive_agent_defensive():
    ...
    case_receive_agent_defensive()
    
    
    # E3: record_agent 缺失 scene_tag 时使用默认值
    def case_record_default_scene_tag():
        import record_agent
    ...
    case_record_default_scene_tag()
    
    
    # E4: workflow 条件路由 - 无效输入不进入 dispatch 和 record
    def case_workflow_routing():
        """验证无效输入在工作流中被正确拦截"""
