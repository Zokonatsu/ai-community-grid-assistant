...
测试范围：
  ...
  2. 注册定位在小区范围内 -> location_status=verified；越界 -> unverified（仍允许注册）
...
# 2. 越界坐标注册 -> 允许注册但 unverified
reg_out = register("res_out", phone="13900000002", lat=OUT_LAT, lng=IN_LNG)
check("2.1 越界坐标允许注册", reg_out.get("success"), str(reg_out))
u_out = reg_out.get("data", {}).get("user", {})
check("2.2 越界坐标 location_status=unverified", u_out.get("location_status") == "unverified", f"loc={u_out.get('location_status')}")
...
check("5.2 列表含 res_in / res_out", "res_in" in names and "res_out" in names, f"names={names}")
